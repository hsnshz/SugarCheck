...
router.post("/glucose/:id", authMiddleware, async (req, res) => {
  try {
...
router.get("/fetch-glucose/:id/:date", authMiddleware, async (req, res) => {
  try {
    // Check if the authenticated user is the same as the user to be updated
    if (req.user.userId !== req.params.id) {
      return res.status(403).send({ error: "Authorization failed." });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const reading = user.healthProfile.glucoseReadings.id(
...
router.get("/fetch-latest-values/:id/", authMiddleware, async (req, res) => {
  try {
    // Check if the authenticated user is the same as the user to be updated
    if (req.user.userId !== req.params.id) {
      return res.status(403).send({ error: "Authorization failed." });
